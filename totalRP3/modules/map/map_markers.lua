...
function launchScan(scanID)
	...
	if true then
		TRP3_API.MapDataProvider:OnScan(TRP3_API.map.getLatestMapScanContent(84))
		return
	end
...
function TRP3_API.map.getLatestMapScanContent(mapID)
	if mapID == 84 then
		local formattedData = {}
		for name, data in pairs(MAP_PLAYER_SCAN_FAKE_DATA.saveStructure) do
